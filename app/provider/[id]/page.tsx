...
async function getProviderListings(providerId: string): Promise<ListingData[]> {
  try {
    const res = await fetch(`${API_URL}/listings?providerId=${providerId}&status=ACTIVE&limit=12`, {
      headers: {
        'Content-Type': 'application/json',
      },
      cache: 'no-store',
            ...
            <div className="text-center md:text-left flex-1">
              ...
              <div className="flex items-center justify-center md:justify-start">
                <div className="bg-green-50 text-green-700 px-5 py-2.5 rounded-xl font-semibold text-sm">
                  {provider._count?.listings || 0} Listing Aktif
                </div>
              </div>
            </div>
          </div>
